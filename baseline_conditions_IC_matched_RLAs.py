...
##                    yield frozenset({edge, edge_})
    ...
    def is_similar_HD_distribution(self,randtree,abs_root):
        ...
        for edgey in rand_tree.edges:
            random_hd_sample.append(find.dependency_depth(edgey))
        random_hd_sample.sort()
        ...
        for edgev in real_tree.edges:
            real_hd_sample.append(get.dependency_depth(edgev))
        real_hd_sample.sort()
        return random_hd_sample==real_hd_sample
    ...
    def rand_tree(self,num_cross_real):                 # requires number of crossings from the real tree
        ...
        if self.is_equal_num_crossings(treey,abstract_root,num_cross_real):  # matches the no. of crossings in the real and random tree
            if self.is_similar_HD_distribution(treey,abstract_root):
    ...
    def gen_random(self,num_cross_real):              # requires numbr of crossings from the real tree
        ...
        if n<30:
            x=0
            while len(self.ls_rand)==0 and x<6000:     # checks if list of random trees is ampty and limits the generating attempts 
                x=x+1
